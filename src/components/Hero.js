...
const Hero = () => {
  const [formData, setFormData] = useState({
    ...
    phone: "",
    eventDate: "", // Will be set to today's date by default
    location: "",
  });

  useEffect(() => {
    // Set today's date as the default event date
    const today = new Date();
    const formattedDate = today.toISOString().slice(0, 7); // Format as YYYY-MM
